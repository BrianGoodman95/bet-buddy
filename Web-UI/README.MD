...
# My understanding of the folders & files in here
- client is everything for the front end of the app
- .env is the env variables essentially to running the application with requied permissions
- .gitignore is the file to tell github what not to add to PRs
- package.json has the versions and info for the core software of the app
- package-lock.json has the versions of the node.js modules/libraries used
- server.js is the main file to start the server and define it's main routes
- db folder has the file to setup the connection to the db
- models folder has the files to define the schema of the databases. Schema includes which fields and their properties
- controllers folder has the files to define the actions of the server for supported request routes. It takes a request and formats it according to a given schema, passes this to the db and passes back a response from the db
- routes folder has the files to define the available routes in the server. This includes the address of the route and what type of communication the route supports (post, get, etc)
- middleware folder has the files to define responses of the server for undefined/error request routes
